...
3. 实现类需要实现 tryAcquire、 tryRelease 等方法。可以先简单的将tryAcquire实现为state通过CAS操作从0变为1，tryRelease实现为state通过CAS操作从1变为0。
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
               ...
               //park阻塞过程中被中断，不会抛出InterruptedException，不会导致异常推出。
           ...
           //failed默认值为空，所以应该是没有正常获取成功，抛出异常退出了。只有tryAcquire方法可能会抛出一场，
           //tryAcquire为子类实现的代码，我们自己都可以写一个类来实现，所以次类中是完全不可控的，一旦捕获到异常，就会执行cancelAcquire。
            if (failed)
                cancelAcquire(node);
        }
    }
...
```
获取逻辑步骤总结：
1. acquire方法为入口，先调用实现类的tryAcquire（将state通过CAS操作从0改为1）
    - 1.1 如果第一个线程来进行获取是会立即成功返回的。
    - 1.2 如果线程来获取时，其它线程在其之前已经获取成功并未释放之前，tryAcquire则失败。
2. addWaiter及enq 来初始化一个Node，并且进行入链。
    - 2.3 如果第一次有note入链（head和tail为null），则会先new一个node作为head和tail（enq中的逻辑）
   ...
   private void doAcquireInterruptibly(int arg)
      ...
      try {
         for (;;) {
            ...
            if (shouldParkAfterFailedAcquire(p, node) &&
                    parkAndCheckInterrupt())
                //这个方法和acquire方法几乎完全一样，只是在这里的逻辑不通：
                //acquire 如果被终端唤醒了，只是将interrupted变量置为了true，然后继续循环，
...
```
总结：
1. 该方法是tryAcquire的扩展，不是acquire的扩展哦，用来实现tryLock(long, TimeUnit)。
